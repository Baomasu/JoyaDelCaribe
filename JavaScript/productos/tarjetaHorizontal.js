...
function updateSize() { //Funcion para actualizar los booleanos de mobile o tablet
    ...
    renderCarouselItems(products); //Volver a renderizar los items del carrusel
}

function getData() { //Obtener datos del JSON
    fetch('../JSONS/productos.json', { method: 'GET' })
        ...
        .then((productObj) => { //Si existe respuesta, llama productObj al nuevo json obtenido
            ...
            renderCarouselItems(products); //Renderiza los items del carrusel
        })
        .catch((error) => {
            console.log('Hubo un problema con el JSON ' + error); 
        });
}
function renderCarouselItems(productList) {
...
// Inicializar el tamaño y cargar los datos
updateSize();
getData();
