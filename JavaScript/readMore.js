...
btnDiana.addEventListener("click", function (event) {
    ...
    if (btnState[0]) {
        diana.innerHTML=`
        <p class="desc-txt">Desarrolladora Full Stack, con intereses de integrarme al mundo del
        desarrollo web ...</p>`;
...
btnEsdras.addEventListener("click", function (event) {
    ...
    if (btnState[1]) {
        esdras.innerHTML=`
        <p class="desc-txt">El desarrollo web me ha permitido desarrollar persistencia y
...
btnAlan.addEventListener("click", function (event) {
    ...
    if (btnState[2]) {
        alan.innerHTML=`
        <p class="desc-txt">Desarrollador Java Full Stack Junior con un fuerte interés en la
        tecnología y la ...</p>`;
        console.log("texto corto");
        btnState[2] = false;
    } else {
...
btnJavier.addEventListener("click", function (event) {
    ...
    if (btnState[3]) {
        javier.innerHTML=`
        <p class="desc-txt">Soy un apasionado desarrollador Full Stack Java Jr con estudios
        en ingeniería de ...</p>`;
        console.log("texto corto");
        btnState[3] = false;
    } else {
...
btnJesus.addEventListener("click", function (event) {
    ...
    if (btnState[4]) {
        jesus.innerHTML=`
        <p class="desc-txt">Profesional y apasionado realizando los proyectos y
...
btnAna.addEventListener("click", function (event) {
    ...
    if (btnState[5]) {
        ana.innerHTML=`
        <p class="desc-txt">Cuento con una alta capacidad para resolver problemas de manera
...
btnBrian.addEventListener("click", function (event) {
    ...
    if (btnState[6]) {
        brian.innerHTML=`
        <p class="desc-txt">¡Hola! Soy analista de datos, desarrollador de video-juegos y
        artista digital en ...</p>`;
        console.log("texto corto");
        btnState[6] = false;
    } else {
        brian.innerHTML=`
        <p class="desc-txt" style="height: 9rem;">¡Hola! Soy analista de datos, desarrollador de video-juegos y artista digital en México. Me gusta la programación creativa para expresarme de forma artística.</p>`;
        console.log("texto largo");
        btnState[6] = true;
    }
});
...
btnAlexia.addEventListener("click", function (event) {
    ...
    if (btnState[7]) {
        alexia.innerHTML=`
        <p class="desc-txt">Soy Alexia Mata, desarrolladora Fullstack con habilidades para
        crear aplicaciones ...</p>`;
        console.log("texto corto");
        btnState[7] = false;
    } else {
        alexia.innerHTML=`
        <p class="desc-txt" style="height: 9rem;">Soy Alexia Mata, desarrolladora Fullstack con habilidades para crear aplicaciones web dinámicas y escalables. Me interesan la ciberseguridad y las redes neuronales artificiales</p>`;
        console.log("texto largo");
        btnState[7] = true;
    }
});